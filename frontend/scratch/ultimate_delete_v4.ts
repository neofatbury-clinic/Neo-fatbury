...
async function ultimateDeleteV4() {
  const filenames = [
    'slimming-hero.png',
    'hair-loss-hero.png',
    ...
    'neofatbury-hair2-banner.webp',
    'neofatbury-slimming-banner.webp',
    'neofatbury-hero-banner.webp',
    'neofatbury-cheek-banner.webp'
  ];

  try {
    // Search for assets by filename or hash prefix
    const assets = await client.fetch(`*[_type == "sanity.imageAsset" && (
      originalFilename in $filenames || 
      originalFilename match "59fbca*" || 
      originalFilename match "9c8f7f*" ||
      originalFilename match "8c6d85*" ||
      originalFilename match "5943f5*"
    )]{ _id, originalFilename }`, { filenames });
