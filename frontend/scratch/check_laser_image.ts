...
async function checkLaser() {
  const data = await client.fetch(`*[_type == "service" && slug.current == "laser-hair-reduction"][0] {
    name,
    heroImage
  }`);
  console.log('Laser Data:', JSON.stringify(data, null, 2));
}
