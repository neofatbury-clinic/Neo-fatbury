...
export default async function AcneScarTreatment() {
  const data = await getServicePageData('acne-scar-treatment');

  // Fallbacks if data is missing
  const heroHeading = data?.heroHeadline || "Smooth Skin.";
  const heroAccent = data?.heroAccentLine || "Zero Acne Scars.";
  const heroDesc = data?.heroSubtext || "Advanced clinical scar restoration using surgical-grade technology.";
  return (
    <>
        ...
        <div className="container">
          <div className="service-hero-grid">
            ...
            <div className="service-hero-text">
              <h1>{heroHeading}<br/><span className="accent">{heroAccent}</span></h1>
              <p>{heroDesc}</p>
